...
<template>
    ...
    <div class="mx-auto max-w-2xl px-4 pb-24 pt-16">
        <div class="rounded-3xl border border-border bg-card card-shadow p-10 text-center">
            <svg
            ...
            <h1 class="text-5xl font-bold !text-ink">{{ FeedbackScripts.heroTitle }}</h1>
            <p class="py-6 !text-ink-light">{{ FeedbackScripts.heroContent }}</p>

            <div class="mb-8 flex flex-wrap justify-center gap-2">
                <span
                    v-for="category in FeedbackScripts.categories"
                    :key="category"
                    class="rounded-full border border-terracotta/30 bg-terracotta/10 px-4 py-1 text-xs font-semibold uppercase tracking-widest !text-ink"
                >
                    {{ category }}
                </span>
            </div>
